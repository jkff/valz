...
public class H2DataStoreBigMaps {
    ...
    public <K, T> void insertBigMapItem(String name, K key, T value) {
        name = name.toUpperCase();

        KeyType<K> keyType = getBigMapKeyType(name);
        Aggregate<T> aggregate = getBigMapAggregate(name);
        insertBigMapItem(name, keyType, aggregate, key, value);
    }
    ...
    public <K, T> void updateBigMapItem(String name, K key, T value) {
        name = name.toUpperCase();

        KeyType<K> keyType = getBigMapKeyType(name);
        Aggregate<T> aggregate = getBigMapAggregate(name);
        updateBigMapItem(name, keyType, aggregate, key, value);
    }

    public <K, T> void updateBigMapItem(String name, KeyType<K> keyType, Aggregate<T> aggregate, K key,
                                        T newValue) {
        name = name.toUpperCase();
    ...
    public void removeBigMap(String name) {
        ...
        database.execute(database.createQuery(String.format("DROP TABLE BM.%s", name)),
                database.createQuery("DELETE BigMaps WHERE name = ?", name));
    }



    private Object getResultSetValue(KeyType keyType, ResultSet resultSet, int i) throws SQLException {
        i = i + 1; // because indicies in resultSet are one-based
        if (keyType instanceof KeyLong) {
            return resultSet.getLong(i);
        } else if (keyType instanceof KeyString) {
